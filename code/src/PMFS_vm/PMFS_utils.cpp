...
namespace GSL
{
    ...
    GSLResult PMFS::checkSourceFound()
    {
        ...
        // CUSUM: Cumulative Sum stability check (from statistical process control).
        // Instead of blocking declaration with an external gate, we improve the declaration
        // criterion itself: the probability peak must be stable for N consecutive iterations.
        // This prevents premature convergence on noise while allowing genuine convergence.
        if (pmfsConvergencePass && settings.review.egs_enabled) {
            // Find current peak location
            int peakIdx = 0;
            double peakVal = 0;
            for (size_t i = 0; i < sourceProbability.size(); i++) {
                if (occupancy[i] == Occupancy::Free && sourceProbability[i] > peakVal) {
                    peakVal = sourceProbability[i];
                    peakIdx = i;
                }
            }
            Vector2 currentPeak = gridMetadata.indicesToCoordinates(peakIdx % (int)gridMetadata.dimensions.x,
                                                                     peakIdx / (int)gridMetadata.dimensions.x);

            // Track peak location history
            review_peak_history_.push_back(currentPeak);

            // CUSUM: check if peak has been stable for min_stable_iterations
            const int min_stable = std::max(3, settings.review.egs_min_iterations);
            bool peakStable = true;
            if ((int)review_peak_history_.size() >= min_stable) {
                for (int k = (int)review_peak_history_.size() - min_stable; k < (int)review_peak_history_.size() - 1; k++) {
                    double dx = review_peak_history_[k].x - review_peak_history_[k+1].x;
                    double dy = review_peak_history_[k].y - review_peak_history_[k+1].y;
                    double dist = std::sqrt(dx*dx + dy*dy);
                    if (dist > 1.0) {  // peak moved more than 1m = not stable
                        peakStable = false;
                        break;
                    }
                }
            } else {
                peakStable = false;  // not enough history yet
            }

            if (!peakStable) {
                GSL_INFO("[CUSUM] Peak not yet stable ({} history entries, need {}), continuing search",
                         review_peak_history_.size(), min_stable);
                return GSLResult::Running;
            }
            GSL_INFO("[CUSUM] Peak stable for {} iterations, allowing declaration", min_stable);
        }
    ...
    void PMFS::saveResultsToFile(GSLResult result)
    {
        ...
        if (settings.simulation.sdr_enabled && hce_hit_count >= settings.simulation.sdr_min_hits && (raw_hasPeakGas || hasPeakGas))
        {
            ...
            if (maxVal > 0.0 && sdr_peak_ratio >= settings.simulation.sdr_min_peak_mass_ratio) {
                ...
                sdr_x = sdrEst.x;
                sdr_y = sdrEst.y;
                // ITS-G: Inertia Tensor Gating (from astronomical image analysis).
