...
namespace GSL
{
    ...
    void MovingStatePMFS::chooseGoalAndMove()
    {
        ...
        for (size_t i = 0; i < pmfs->sourceProbability.size(); i++)
        {
            ...
            // IFT: Use distance-biased exploration when EGS is active and no gas hits
            // This prevents the UAV from getting stuck in a local area
            bool useIFT = pmfs->settings.review.eae_enabled && pmfs->review_hit_archive_.empty() && pmfs->iterationsCounter > 3;
            if (useIFT) {
                explorationTerm = iftExplorationValue(indices.x, indices.y);
            }
...
#pragma omp parallel for
    ...
    double MovingStatePMFS::iftExplorationValue(int i, int j)
    {
        // Information Foraging Theory: when local evidence is insufficient,
        // bias exploration toward distant unexplored regions.
        // This prevents the UAV from getting stuck in a local area.
        Vector2Int ij(i, j);
        auto range = pmfs->visibilityMap->at(ij);

        double localExploration = 0;
        for (const auto& p : range)
        {
            float distance = vmath::length(Vector2(ij - p));
            localExploration += (1 - pmfs->hitProbability[pmfs->gridMetadata.indexOf(p)].confidence) * std::exp(-distance);
        }

        // Distance from current position - farther cells get bonus
        Vector2 currentPos(pmfs->currentRobotPose.pose.pose.position.x, pmfs->currentRobotPose.pose.pose.position.y);
        Vector2 cellPos = pmfs->gridMetadata.indicesToCoordinates(i, j);
        float distFromRobot = vmath::length(cellPos - currentPos);

        // IFT combination: local exploration + distance bonus
        // The distance bonus encourages the UAV to visit far-away areas
        double distanceBonus = std::log(1.0 + distFromRobot / 2.0);
        return localExploration + 0.5 * distanceBonus;
    }
