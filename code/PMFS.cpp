...
// DISABLED_OLD_PWC:         }
    ...
    void PMFS::declareParameters()
    {
        ...
        settings.review.eae_enabled = getParam<bool>("eae_enabled", false);
        settings.review.eae_base_exploration_prob = getParam<double>("eae_base_exploration_prob", 0.05);
        settings.review.eae_max_exploration_prob = getParam<double>("eae_max_exploration_prob", 0.50);
        settings.review.eae_boost_per_miss = getParam<double>("eae_boost_per_miss", 0.03);
        settings.review.eae_miss_threshold = getParam<int>("eae_miss_threshold", 3);
        settings.review.eae_extend_search_on_low_evidence = getParam<bool>("eae_extend_search_on_low_evidence", true);
        settings.review.eae_max_extension_factor = getParam<double>("eae_max_extension_factor", 2.0);
...
namespace GSL
{
    ...
    void PMFS::processGasAndWindMeasurements(double concentration, double windSpeed, double windDirection)
    {
        ...
        // EAE: update evidence-adaptive exploration
        if (settings.review.eae_enabled) {
            if (concentration > thresholdGas) {
                eae_miss_count_ = 0;
                eae_last_hit_iteration_ = iterationsCounter;
            } else {
                eae_miss_count_++;
            }
            // Adapt exploration probability based on evidence
            double boost = eae_miss_count_ * settings.review.eae_boost_per_miss;
            eae_current_exploration_prob_ = std::min(
                settings.review.eae_max_exploration_prob,
                settings.review.eae_base_exploration_prob + boost);
            // Dynamically set the exploration probability in movement settings
            settings.movement.explorationProbability = eae_current_exploration_prob_;
            // Extend max search time if evidence is weak
            if (settings.review.eae_extend_search_on_low_evidence && review_hit_archive_.size() < (size_t)settings.review.eae_miss_threshold) {
                resultLogging.maxSearchTime = eae_original_max_search_time_ * settings.review.eae_max_extension_factor;
            }
        }

        // EAE-FR: Frontier Reach - increase exploration radius when evidence is weak
        if (settings.review.eae_enabled && review_hit_archive_.empty() && iterationsCounter > 3) {
            // Double the open move set expansion to reach farther cells
            int extra = std::min(10, (int)(iterationsCounter - 3));
            settings.movement.openMoveSetExpasion = 5 + extra;
            if (settings.review.review_modules_verbose && iterationsCounter % 3 == 0) {
                GSL_INFO("[EAE-FR] No hits after {} iters, expanding move set to {}",
                         iterationsCounter, settings.movement.openMoveSetExpasion);
            }
        }
