...
EXPECTED_BRANCH="research/path-action-source-inference-v0"
...
OUT_ROOT="${OUT_ROOT:-/home/zyc/pasi_d0_s3_w2_20260924}"
...
SY="-3.700880051"
SZ="0.20"
TARGET_SEEDS=(2026092403 2026092404)
TARGET_NAMES=(S3_W2_E S3_W2_F)
...
r=d[d.source_id=="pmfs_3_12"]
assert len(r)==1
r=r.iloc[0]
assert abs(float(r.x_m)-(-4.342730045))<1e-5
assert abs(float(r.y_m)-(-3.700880051))<1e-5
assert abs(float(r.z_m)-0.20)<1e-8
print("S3 truth support verified:",r.source_id,r.x_m,r.y_m,r.z_m)
PY
...
set +e
python3 "$RESEARCH/score_pasi_d0_s3.py"   --contract "$GATE1A_ROOT/gate1a_contract.json"   --source-bank "$GATE1A_ROOT/source_bank.tsv"   --prediction-root "$GATE1A_ROOT/predictions"   --target-e "$OUT_ROOT/S3_W2_E/spatial/concentration.npy"   --target-f "$OUT_ROOT/S3_W2_F/spatial/concentration.npy"   --truth-source-id "$TRUTH_ID"   --out "$RESULT" | tee "$EVIDENCE/PASI_D0_S3_W2_RUN_20260924.log"
rc=${PIPESTATUS[0]}
set -e

{
  git -C "$ROOT" rev-parse HEAD
  sha256sum     "$RESEARCH/score_pasi_d0_s3.py"     "$ROOT/research/path_action_source_inference_v0/run_pasi_d0_s3_w2_vm.sh"     "$GATE1A_ROOT/source_bank.tsv"     "$GATE1A_ROOT/gate1a_contract.json"     "$OUT_ROOT/S3_W2_E/spatial/concentration.npy"     "$OUT_ROOT/S3_W2_F/spatial/concentration.npy"     "$RESULT"
} > "$EVIDENCE/PASI_D0_S3_W2_SHA256_20260924.txt"
