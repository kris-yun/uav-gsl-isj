...
{
    try {
        if (argc != 5) throw std::runtime_error("usage: native_pmfs_r1_forward_replay SNAPSHOT_DIR ARM OUTPUT_DIR STATE_INPUT_DIR_OR_DASH");
        cv::setNumThreads(1);
        fs::path run(argv[1]), out(argv[3]), states(argv[4]);
        const bool generateBank = states != fs::path("-");
        const std::string arm(argv[2]);
        if (arm != "C") throw std::runtime_error("arm must be A/B/C");
        ...
        std::vector<std::vector<Vector2>> stateWinds;
        if (generateBank) {
