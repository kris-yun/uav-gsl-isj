...
OUT_ROOT="${OUT_ROOT:-/home/zyc/cess_d1_630x8_20260924}"
...
HIER="$EVIDENCE/CESS_D1_HIERARCHY.json"
MERGES="$EVIDENCE/CESS_D1_HIERARCHY_MERGES.tsv"
...
for f in "$SOURCE_BANK" "$CONTRACT"; do
  [[ -f "$f" ]] || { echo "missing frozen input $f" >&2; exit 5; }
done

python3 - "$SOURCE_BANK" <<'PY'
import sys,pandas as pd
d=pd.read_csv(sys.argv[1],sep="\t")
assert len(d)==630 and d.source_id.nunique()==630
...
inventory="$EVIDENCE/CESS_D1_ARTIFACT_SHA256.tsv"
printf "source_index\tsource_id\treplicate\trng_seed\tconcentration_sha256\tpooled_sha256\n" > "$inventory"

python3 - "$SOURCE_BANK" <<'PY' > "$OUT_ROOT/source_rows.tsv"
import sys,pandas as pd
d=pd.read_csv(sys.argv[1],sep="\t")
for i,r in d.iterrows():
    print(i,r.source_id,repr(float(r.x_m)),repr(float(r.y_m)),repr(float(r.z_m)),sep="\t")
PY

while IFS=$'\t' read -r source_index source_id sx sy sz; do
  for rep in $(seq 1 8); do
    seed=$((2026100000 + 8*source_index + rep))
...
PY
    ...
    if [[ "$valid" -eq 0 ]]; then
      ...
      echo "RUN source=$source_index/629 $source_id rep=$rep seed=$seed"
      ...
      "$EXTRACTOR" \
        "$real" "$real" \
        "$spatial/concentration.npy" "$spatial/metadata.json" \
...
a=np.stack(vals,axis=1).astype(np.float64)
...
EOF
      ...
      echo "SKIP valid source=$source_index $source_id rep=$rep seed=$seed"
    fi

    csha="$(sha256sum "$spatial/concentration.npy" | awk '{print $1}')"
    psha="$(sha256sum "$pooled" | awk '{print $1}')"
    printf "%s\t%s\t%s\t%s\t%s\t%s\n" \
      "$source_index" "$source_id" "$rep" "$seed" "$csha" "$psha" >> "$inventory"
  done
done < "$OUT_ROOT/source_rows.tsv"

[[ "$(tail -n +2 "$inventory" | wc -l)" -eq 5040 ]] || {
  echo "artifact inventory incomplete" >&2; exit 22;
}

set +e
python3 "$RESEARCH/analyze_cess_d1.py" \
  --source-bank "$SOURCE_BANK" \
  --hierarchy-json "$HIER" \
  --data-root "$OUT_ROOT" \
  --out-dir "$EVIDENCE" \
  | tee "$EVIDENCE/CESS_D1_ANALYSIS_RUN.log"
rc=${PIPESTATUS[0]}
set -e
...
} > "$EVIDENCE/CESS_D1_GIT_STATE.txt"
...
sha256sum \
  ...
  "$CONTRACT" \
  "$HIER" \
  "$EVIDENCE/CESS_D1_RESULT.json" \
  > "$EVIDENCE/CESS_D1_FROZEN_SHA256.txt"

cat "$EVIDENCE/CESS_D1_RESULT.json"
exit "$rc"
