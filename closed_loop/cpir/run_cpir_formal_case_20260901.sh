...
# House-aware formal CPIR/PMFS paired runner.
#
...
CPIR_LAUNCH_FILE="${CPIR_LAUNCH_FILE:-${REPO_ROOT}/closed_loop/cpir/vgr_gsl_pmfs_cpir.launch.py}"
CPIR_PREFLIGHT_SCRIPT="${CPIR_PREFLIGHT_SCRIPT:-${REPO_ROOT}/tools/cpir_formal_preflight.py}"
...
RUN_ID="${RUN_ID:-CPIR_${HSHORT}_S${SEED}_${ARM}_$(date -u +%Y%m%dT%H%M%SZ)}"
RUN_DIR="${RUN_ROOT}/${HSHORT}_seed${SEED}_${ARM}"
CPIR_AUDIT_DIR="${RUN_DIR}/cpir_audit"
PREFLIGHT_JSON="${RUN_DIR}/cpir_formal_preflight.json"
ALGORITHM_BINARY="${PFDI_INSTALL_ROOT}/install/gsl_server/lib/gsl_server/gsl_actionserver_node"

for path in "${CPIR_LAUNCH_FILE}" "${CPIR_PREFLIGHT_SCRIPT}" "${ALGORITHM_BINARY}" "${INTEGRITY_REPORT}"; do
...
{
  "contract": "CPIR_FORMAL_PAIRED_RUN_V1",
  "run_id": "${RUN_ID}",
  "house": "${HSHORT}",
  "seed": ${SEED},
  "arm": "${ARM}",
...
ARGS=(
  ...
  "method:=CPIR_THREE_MODULE_NESTED" "method_family:=cpir_nested" "ablation_id:=${ARM}"
...
echo "CPIR_FORMAL_CASE_START HOUSE=${HSHORT} SEED=${SEED} ARM=${ARM} MODE=${PFDI_MODE} DOMAIN=${ROS_DOMAIN_ID}"
set +e
setsid ros2 launch "${CPIR_LAUNCH_FILE}" "${ARGS[@]}" >"${RUN_DIR}/launch.log" 2>&1 &
launch_pid=$!
...
if [[ ! -s "${RUN_DIR}/run_status.json" ]]; then
  echo "CPIR_FORMAL_CASE_NO_TERMINAL_STATUS HOUSE=${HSHORT} SEED=${SEED} ARM=${ARM} STATUS=${status}" >&2
  exit "${status:-1}"
fi

echo "CPIR_FORMAL_CASE_COMPLETE HOUSE=${HSHORT} SEED=${SEED} ARM=${ARM} RUN_DIR=${RUN_DIR}"
