...
This directory prevents a labelled CTPI V0.4 run from silently executing the inherited CPIR runtime.
...
3. V0.4 APRS must not multiply the same current measured-hit evidence onto a native PMFS posterior that already consumed it. Runtime integration must implement a replacement/ownership boundary for the same observation update.
4. V0.4 PSRG is a parallel metrology output. It must not be inserted into the source posterior or planner as an unregistered weighting term.
5. The current PMFS movement score primarily uses `varianceOfHitProb * (1-confidence)`. A formal V0.4 closed loop must explicitly prove that the frozen V0.4 posterior used at update `t` is the posterior handed to the next action decision. Do not claim this from node liveness alone.

## Required runtime handshake

The actual C++/ROS implementation must deliberately expose all three strings below so that source and binary provenance can be checked:

- runtime mode: `ctpi_v04`
- runtime contract marker: `CTPI_V04_RUNTIME_CONTRACT_V1`
- posterior entry point marker: `applyCTPIV04Posterior`

These are engineering provenance markers, not new scientific parameters.
...
While the runtime port is absent, exit 42 / `BLOCKED_BY_RUNTIME_PARITY` is the correct behavior. Do not bypass it by mapping V0.4 to a CPIR mode.
