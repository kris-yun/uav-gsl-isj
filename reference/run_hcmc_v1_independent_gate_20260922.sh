...
# Frozen HCMC-V1 independent-plume Native-trajectory runner.
# This is an execution-only derivative of run_meaci_case_20260824.sh.  It
# adds explicit realization binding and provenance while retaining ARM=off,
# PFDI_MODE=off, TNQC_MODE=off and the historical Native PMFS launch contract.

HOUSE="${HOUSE:?set HOUSE=House01, House02, or House03}"
SEED="${SEED:?set SEED=0 or 1}"
ARM="${ARM:?set ARM=off or on}"
[[ "${ARM}" == "off" ]] || { echo "HCMC independent gate requires ARM=off" >&2; exit 2; }
REALIZATION_ID="${REALIZATION_ID:?set frozen independent realization ID}"
REALIZATION_ROOT="${REALIZATION_ROOT:?set frozen independent realization directory}"
[[ -d "${REALIZATION_ROOT}" ]] || { echo "missing realization: ${REALIZATION_ROOT}" >&2; exit 2; }
...
RUN_DIR="${RUN_ROOT}/${REALIZATION_ID}"
RUN_ID="${RUN_ID:-HCMC_NATIVE_${REALIZATION_ID}_NAV${SEED}_$(date -u +%Y%m%dT%H%M%SZ)}"
...
REALIZATION_MANIFEST="$(dirname "${REALIZATION_ROOT}")/generation_manifest.txt"
[[ -f "${REALIZATION_MANIFEST}" ]] || { echo "missing realization manifest: ${REALIZATION_MANIFEST}" >&2; exit 2; }
REALIZATION_MANIFEST_SHA256="$(sha256sum "${REALIZATION_MANIFEST}" | awk '{print $1}')"
...
mkdir -p "${RUN_DIR}"
CANONICAL_VGR_DATA="${VGR_DATA}"
if [[ "${HOUSE}" == "House02" || "${HOUSE}" == "House03" ]]; then
  # vgr_sim_node discovers max_iteration from vgr_data_path.  Bind a
  # case-local read-only scenario view so its replay modulus is derived from
  # the new realization, not from the 2000-frame discovery realization.
  SCENARIO_BIND="${RUN_DIR}/scenario_bind/${HOUSE}"
  mkdir -p "${SCENARIO_BIND}/gas_simulations/${CONFIG_ID}"
  ln -s "${CANONICAL_VGR_DATA}/OccupancyGrid3D.csv" "${SCENARIO_BIND}/OccupancyGrid3D.csv"
  ln -s "${CANONICAL_VGR_DATA}/occupancy.yaml" "${SCENARIO_BIND}/occupancy.yaml"
  ln -s "${CANONICAL_VGR_DATA}/occupancy.pgm" "${SCENARIO_BIND}/occupancy.pgm"
  ln -s "${CANONICAL_VGR_DATA}/wind_simulations" "${SCENARIO_BIND}/wind_simulations"
  ln -s "${REALIZATION}" "${SCENARIO_BIND}/gas_simulations/${CONFIG_ID}/$(basename "${REALIZATION}")"
  VGR_DATA="${SCENARIO_BIND}"
fi
...
{
  "contract": "${RUN_CONTRACT:-HCMC_V1_INDEPENDENT_NATIVE_TRAJECTORY_V1}",
  "run_id": "${RUN_ID}",
  "house": "${HOUSE}",
  "seed": ${SEED},
  "arm": "${ARM}",
  ...
  "realization_manifest_sha256": "${REALIZATION_MANIFEST_SHA256}",
  "canonical_vgr_data": "${CANONICAL_VGR_DATA}",
  "bound_vgr_data": "${VGR_DATA}",
...
export PYTHONPATH="/opt/ros/humble/local/lib/python3.10/dist-packages:/opt/ros/humble/lib/python3.10/site-packages:/dev/shm/house1_msgs_install/local/lib/python3.10/dist-packages:/home/zyc/hcmc_gaden_seed_build_20260922/install/gaden_msgs/local/lib/python3.10/dist-packages:/dev/shm/house1_vgr_bridge:/home/zyc/ros2_ws/src/vgr_bridge:${PYTHONPATH:-}"
if [[ "${HOUSE}" == "House02" || "${HOUSE}" == "House03" ]]; then
  source /home/zyc/hcmc_gaden_seed_build_20260922/install/gaden_msgs/share/gaden_msgs/local_setup.bash
  source /home/zyc/hcmc_gaden_seed_build_20260922/install/gaden_common/share/gaden_common/local_setup.bash
  source /home/zyc/hcmc_gaden_seed_build_20260922/install/gaden_player/share/gaden_player/local_setup.bash
fi
set -u
if [[ -n "${GMRF_PREFIX}" ]]; then
...
if [[ "${HOUSE}" == "House02" || "${HOUSE}" == "House03" ]]; then
  ...
  nohup /home/zyc/hcmc_gaden_seed_build_20260922/install/gaden_player/lib/gaden_player/player --ros-args -r __node:=gaden_player \
...
ARGS=(
  ...
  "sensor_model_mode:=dynamic" "gaden_iteration_mode:=seeded_time_replay" "realtime_factor:=${REALTIME_FACTOR:-1.0}" "sim_stop_at_s:=${SIM_STOP_AT_S:-300.0}" "nav_command_quantum_s:=${NAV_COMMAND_QUANTUM_S:-2.0}"
  "gas_backend:=${GAS_BACKEND}" "raw_query_executable:=${RAW_QUERY}"
  "raw_env_root:=${VGR_DATA}" "raw_gas_results:=${REALIZATION}"
