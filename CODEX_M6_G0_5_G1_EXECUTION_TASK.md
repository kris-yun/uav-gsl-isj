> **RNG BLOCKER:** Read `evidence/geopt_physics_foundation_pmfs_v1/G1_GADEN_RNG_SEED_CONTROL_AUDIT_20260923.md`. Current public gaden_core exposes no obvious seed parameter. Do not label new runs “seed11/12” until the exact stochastic seed path is proven. Same-seed reproducibility and different-seed divergence are required before 12×2 generation.\n\n> **G1 TARGET CONTRACT:** Read `evidence/geopt_physics_foundation_pmfs_v1/G1_HOUSE02_HIGH_FIDELITY_TARGET_CONTRACT_20260923.md`. Primary House02 supervision is the GADEN hit-frequency field at sensor z=0.30 m, threshold 0.1, 50 s burn-in, 1 Hz sampling over t=50..299 s. Do not replace this target/window after inspecting truth rank.\n\n> **PMFS CANDIDATE-SEMANTICS UPDATE:** Also read `evidence/geopt_physics_foundation_pmfs_v1/G0_9_QUADTREE_SOURCE_INTEGRATION_20260923.md`. In the hard source-rank replay, a PMFS quadtree candidate is a source **region**, not a center point. Primary M6 inference must average the point-conditioned forward model over the fixed 2×2 Gauss-Legendre quadrature points of each candidate rectangle. Center-only is diagnostic/ablation only and must not be selected by truth rank.
