...
free grid cells, then probability-weighted coordinate mean. Native PMFS
posterior snapshots are resolved by the archive's authoritative source-update
IDs, never by assuming that model update ordinals equal native update IDs.
...
from pf_snre_spatial_eval import (
    load_carrier_cells, expand_region_mass_to_cells,
    spatial_energy_score, precompute_distance_matrix,
)
    ...
    with path.open(newline="",encoding="utf-8-sig") as f:
        for r in csv.DictReader(f):
            if r["house"] in (house,HOUSE_LONG[house]):
                ...
                if seed in out:
                    raise ValueError(f"duplicate truth {house} seed{seed}")
                out[seed]=xy
    if set(out)!=set(range(10)):
        raise ValueError(f"{house}: truth CSV must contain seeds0..9 exactly")
    return out
...
def _observed_blocks(path: Path, house: str):
    ...
    with path.open(newline="",encoding="utf-8-sig") as f:
        for r in csv.DictReader(f):
            if r["house"] not in (house,HOUSE_LONG[house]):
                continue
            seed=int(r["seed"])
            if seed not in out:
                continue
            ...
            if idx.shape!=(10,) or y.shape!=(10,):
                raise ValueError("observed block must have 10 indices/measurements")
            out[seed].append((int(r["block_id"]),idx,y))
    for seed,rows in out.items():
        rows.sort(key=lambda z:z[0])
        if [r[0] for r in rows] != list(range(1,len(rows)+1)):
            raise ValueError(f"{house} seed{seed}: incomplete observed blocks")
    return out

def _load_native_posterior(runtime_root: Path, native_update_id: int, cells: np.ndarray):
    name=f"native_shadow_source_posterior_update_{native_update_id:04d}.csv"
    ...
    if len(rows)!=len(cells):
        raise ValueError("native PMFS free-cell count mismatch")
    pos=np.asarray([[r[0],r[1]] for r in rows],dtype=np.float64)
    prob=np.asarray([r[2] for r in rows],dtype=np.float64)
    if np.any(prob<0) or not np.all(np.isfinite(prob)) or prob.sum()<=0:
        raise ValueError("invalid native PMFS posterior")
    mapped=np.empty(len(cells),dtype=np.float64)
    used=set()
    for i,xy in enumerate(cells):
        d=np.linalg.norm(pos-xy[None,:],axis=1)
        k=int(np.argmin(d))
        if d[k]>1e-8 or k in used:
            raise ValueError("native PMFS/free-cell coordinate mismatch")
        mapped[i]=prob[k]
        used.add(k)
    mapped/=mapped.sum()
...
def _model(path: Path, house: str):
    ...
    if (ck.get("heldout_house")!=house or ck.get("seed")!=3301 or
...
def main():
    ...
    truth=_truth(args.truth_csv,args.house)
    observed=_observed_blocks(args.observed_block_manifest,args.house)
    ids,_,q0,cells,ptr=load_carrier_cells(args.carriers)
    q0cell=expand_region_mass_to_cells(q0,ptr)
    ...
    for seed in range(10):
        ...
        if len(specs)<EXPECTED_UPDATES:
            raise ValueError(f"{args.house} seed{seed}: fewer than {EXPECTED_UPDATES} authoritative source updates")
        authoritative=specs[:EXPECTED_UPDATES]
        native_ids=[int(spec.update_id) for spec in authoritative]
        if len(set(native_ids))!=EXPECTED_UPDATES or any(
                native_ids[i] >= native_ids[i+1] for i in range(EXPECTED_UPDATES-1)):
            raise ValueError(f"{args.house} seed{seed}: invalid authoritative update-ID sequence {native_ids}")
        update_maps[str(seed)]=native_ids

        for ordinal,spec in enumerate(authoritative,start=1):
            native_update_id=int(spec.update_id)
            visible=data.blocks.visible(seed,ordinal)
            if len(observed[seed])<visible:
                raise ValueError(f"{args.house} seed{seed}: observed blocks shorter than canonical prefix")
            oi=np.stack([r[1] for r in observed[seed][:visible]])
            ci=data.blocks.sample_indices[seed,:visible]
            if not np.array_equal(oi,ci):
                raise ValueError(f"{args.house} seed{seed}: canonical/observed block index mismatch")
            y=np.stack([r[2] for r in observed[seed][:visible]])
            full=np.zeros(data.measured.shape[-1],dtype=np.float32)
            flat_i=ci.reshape(-1)
            full[flat_i]=y.reshape(-1)

            logits=score_candidates(model,data,full,seed,visible)
            post=direct_posterior(q0,logits)
            cellp=expand_region_mass_to_cells(post,ptr)
            native,native_path=_load_native_posterior(run.runtime_root,native_update_id,cells)
            native_files.append(str(native_path))
            ...
            rows.append({
                "house":args.house,
                "seed":seed,
                "source_update_ordinal":ordinal,
                "native_source_update_id":native_update_id,
                "visible_blocks":visible,
                "pf_expectedvalue05_error_m":pf_err,
                "pmfs_expectedvalue05_error_m":pmfs_err,
                "q0_expectedvalue05_error_m":q0_err,
                ...
                "pf_max_carrier_mass":float(post.max()),
                "pf_max_cell_mass":float(cellp.max()),
                "pf_top05_selected_cells":pf_meta["selected_cells"],
                "pmfs_top05_selected_cells":pmfs_meta["selected_cells"],
            })
    ...
    by_seed={
        str(seed):{
            "pf":float(np.mean([r["pf_expectedvalue05_error_m"] for r in rows if r["seed"]==seed])),
            "pmfs":float(np.mean([r["pmfs_expectedvalue05_error_m"] for r in rows if r["seed"]==seed])),
            "native_source_update_ids":update_maps[str(seed)],
        } for seed in range(10)
    }
    summary={
        "contract":"PF_SNRE_HISTORICAL_LOHO_V2",
        "house":args.house,
        "cases":len(rows),
        "primary_metric":"native_PMFS_ExpectedValue_sourceProbability_0.05_top_cell_fraction",
        "native_update_resolution":"materialize_v4_truthblind_contexts.reconstruct_context_specs exact update_id",
        "mean_pf_error_m":mean("pf_expectedvalue05_error_m"),
        "mean_pmfs_error_m":mean("pmfs_expectedvalue05_error_m"),
        "relative_improvement_vs_pmfs":(
            mean("pmfs_expectedvalue05_error_m")-mean("pf_expectedvalue05_error_m")
        )/mean("pmfs_expectedvalue05_error_m"),
        "paired_cases_improved":int(sum(
            r["pf_expectedvalue05_error_m"]<r["pmfs_expectedvalue05_error_m"] for r in rows)),
        "by_seed":by_seed,
        "checkpoint_sha256":sha256(args.checkpoint),
        "checkpoint_step":int(ck["step"]),
        "native_posterior_files":native_files,
    ...
    (args.out/"historical_summary.json").write_text(
