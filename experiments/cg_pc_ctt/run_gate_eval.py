...
def scalar_string(d,key,default):
    ...
    x=np.asarray(d[key])
    if x.ndim==0: return str(x.item())
    if x.size==1: return str(x.reshape(-1)[0])
...
def main():
    ...
    p.add_argument('--permutation-seed',type=int,default=20260827)
    p.add_argument('--ridge-rel',type=float,default=1e-3)
    p.add_argument('--member-semantics',choices=['exchangeable_realizations','fixed_nuisance_design'],default=None)
    a=p.parse_args()
    ...
    N=phi.shape[0]
    context=opt(d,'context',N,''); split=opt(d,'split',N,''); house=opt(d,'house',N,''); margin=opt(d,'margin',N,np.nan).astype(float)
    semantics=a.member_semantics or scalar_string(d,'member_semantics','UNDECLARED')
    if semantics=='UNDECLARED': raise ValueError('member_semantics must be declared in NPZ or CLI before evaluation')
    cfg=GateConfig(gamma_min=a.gamma_min,p_max=a.p_max,rank_k=a.rank_k,n_folds=4,n_permutations=a.n_permutations,permutation_seed=a.permutation_seed,ridge_rel=a.ridge_rel,member_semantics=semantics)
    rows=[]
    for i in range(N):
        # Context-specific seed makes permutation null deterministic but distinct.
        ci=GateConfig(**{**cfg.__dict__,'permutation_seed':a.permutation_seed+i})
        g=compute_gate(phi[i],ci)
    ...
    summary={'protocol':'CG_PC_CTT_GATE_V2_CROSSFIT','n':N,'coverage':float(accepted.mean()),'accepted':int(accepted.sum()),'rejected':int((~accepted).sum()),'gamma_min':a.gamma_min,'p_max':a.p_max,'rank_k':a.rank_k,'n_permutations':a.n_permutations,'member_semantics':semantics,'inferential_valid':bool(all(r['inferential_valid'] for r in rows))}
