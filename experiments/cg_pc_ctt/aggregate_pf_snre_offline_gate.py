...
def load(path,contract):
    ...
    if x.get("contract")!=contract:raise ValueError(f"{path}: expected {contract}")
    return x
...
def main():
    ...
    sw=[load(p,"PF_SNRE_RESERVED_SOURCE_SWEEP_V2") for p in a.source_sweeps];hi=[load(p,"PF_SNRE_HISTORICAL_LOHO_V1") for p in a.historical];ca=[load(p,"PF_SNRE_Q0_CALIBRATION_V1") for p in a.calibration]
    ...
    # Each V2 sweep uses the same number (199) of preregistered permutations.
    null=[]
    if all("null_statistics" in sw[h]["source_label_permutation_test"] for h in HOUSES):
        ...
        # Fail closed rather than substituting per-House p-values for the pooled preregistered test.
