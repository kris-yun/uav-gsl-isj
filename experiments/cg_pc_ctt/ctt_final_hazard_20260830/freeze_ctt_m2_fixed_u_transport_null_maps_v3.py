#!/usr/bin/env python3
"""Freeze valid-block-aware destruction maps for the prospective CTT M2 Gate."""
...
CONTRACT = "CTT_M2_FIXED_U_PROSPECTIVE_K_NULL_MAPS_V3"
REPLICATES = 256
SOURCE_COUNT = 210
PREDICTIVE_K_COUNT = 6
ROUTES = (4004, 4005)
# Frozen schedule hashes yield nine and ten complete 80-sample blocks.
STOP_COUNT_BY_ROUTE = {4004: 9, 4005: 10}
SEEDS = {
...
def nonidentity_permutation(rng: np.random.Generator, size: int) -> list[int]:
    ...
    raise RuntimeError("CTT_M2_NULL_MAP_V3_NONIDENTITY_EXHAUSTED")
...
def build_payload() -> dict:
    ...
    for _ in range(REPLICATES):
        stop_maps.append({
            str(route): nonidentity_permutation(stop_rng, STOP_COUNT_BY_ROUTE[route])
            for route in ROUTES
        })
        k_maps.append({
            str(route): [
                nonidentity_permutation(k_rng, PREDICTIVE_K_COUNT)
                for _ in range(STOP_COUNT_BY_ROUTE[route])
            ]
            for route in ROUTES
        })
    return {
        "contract": CONTRACT,
        "status": "FROZEN_AFTER_MATERIALIZATION_BEFORE_ANY_SCORE_OR_OUTCOME_READ",
        "supersedes": (
            "CTT_M2_FIXED_U_PROSPECTIVE_K_NULL_MAPS_V2 solely because V2 "
            "counted incomplete or overlong stop IDs rather than complete "
        ...
        "routes": list(ROUTES),
        "stop_count_by_route": {
            str(route): STOP_COUNT_BY_ROUTE[route] for route in ROUTES
        },
        "seeds": SEEDS,
...
def validate(payload: dict) -> None:
    if payload.get("contract") != CONTRACT or payload.get("replicates") != REPLICATES:
        raise ValueError("CTT_M2_NULL_MAP_V3_CONTRACT")
...
def main() -> int:
    ...
    if args.selftest:
        if encoded != canonical_bytes(build_payload()):
            raise AssertionError("CTT_M2_NULL_MAP_V3_NONDETERMINISTIC")
        print(f"CTT_M2_FIXED_U_TRANSPORT_NULL_MAP_V3_SELFTEST=PASS sha256={digest}")
        return 0
    if args.output is None:
        parser.error("--output is required unless --selftest is used")
    if args.output.exists():
        raise SystemExit(f"CTT_M2_NULL_MAP_V3_REFUSE_OVERWRITE:{args.output}")
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_bytes(encoded)
    print(f"CTT_M2_FIXED_U_TRANSPORT_NULL_MAP_V3_FREEZE=PASS sha256={digest}")
    return 0
