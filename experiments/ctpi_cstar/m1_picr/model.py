...
class PICRModel(nn.Module):
    ...
    def score_source(self, z_s, candidate_xy):
        """Candidate score using zS only; zero zS gives a uniform belief.

        The opt-in radial repair preserves distance curvature. Subtracting
        two equal-width quadratics cancels it and cannot localize an interior
        source. A nonnegative zS-dependent precision instead vanishes at zero
        zS and retains an interior mode for nonzero zS. This is an architectural
        repair, not evidence of causal identification or a novelty claim.
        """
        if self.coordinate_equivariant:
            ...
            if self.radial_source_score:
                energy = z_s.square().mean(-1, keepdim=True)
                return (energy / (1.0 + energy)) * raw
        ...
        zs = z_s[:, None, :].expand(-1, candidate_xy.shape[1], -1)
        return (self.evidence(torch.cat([zs, cand], dim=-1)).squeeze(-1)
                - self.evidence(torch.cat([torch.zeros_like(zs), cand], dim=-1)).squeeze(-1))
    ...
    def forward(self, history: torch.Tensor, candidate_xy: torch.Tensor,
        ...
        logits = self.score_source(z_s, candidate_xy)
