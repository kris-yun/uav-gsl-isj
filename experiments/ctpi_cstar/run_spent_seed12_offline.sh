...
M3_PANEL="${M3_PANEL:-}"
mkdir -p "${OUT_ROOT}"
python3 "${REPO_ROOT}/experiments/ctpi_cstar/build_spent_manifest.py" --run-root "${RUN_ROOT}" --output "${OUT_ROOT}/SPENT_EPISODES.json"
args=(--manifest "${OUT_ROOT}/SPENT_EPISODES.json" --out-dir "${OUT_ROOT}/gates")
if [[ -n "${M3_PANEL}" ]]; then args+=(--m3-panel "${M3_PANEL}"); fi
