...
Status: **THEORY FROZEN / IMPLEMENTATION PARTIALLY ALIGNED / CLOSED LOOP NOT YET AUTHORIZED**
...
1. **M1 — Causal Stochastic Physical Intervention (CSPI)**: generate candidate-conditioned native physical concentration responses under `do(S=s)` and marginalize transport as an exchangeable nuisance ensemble.
...
The current branch base is `codex/ctt-wonline-sufficiency-audit-20260830` at commit `ea450ffc84ce4607988d0deb2867618acb1fa6d7`.
...
- eight predictive transport members;
- `dt = 0.2 s`;
...
However, its final source score is currently **count-collapsed**. It computes a stop-specific predictive probability first, then averages those probabilities over stops and scores only the total number of observed hit stops.
...
The current runtime does **not** yet implement the paper-level M3 formula in Section 6, and its current carrier-to-cell lifting does not yet implement the information-projection interface in Section 8.
...
The current CPIR runtime then introduces another information bottleneck: after computing a predictive detection probability for every stop, it averages those probabilities and uses only the **number** of hit stops. It discards **which physical stops** were hits and which were non-hits.
