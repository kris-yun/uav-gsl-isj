...
RUN_ROOT=${CSTAR_CER_RUN_ROOT:-/mnt/hgfs/workspace/CSTAR_CER_RATIO_M12_HOUSE123_SEED12_20260908}
BUILD_ROOT=/dev/shm/cstar_pmfs_cer_ratio_build_r2_20260908
...
for HOUSE in H01 H02 H03; do
  for ARM in A0 M1R M1M2R; do
    HOUSE="$HOUSE" SEED=12 ARM="$ARM" RUN_ROOT="$RUN_ROOT" REPO_ROOT="$REPO_ROOT" \
      ...
      DOMAIN_ID=226 TIMEOUT_SEC=240.0 METHOD=CTPI_G2_M1_M2 METHOD_FAMILY=ctpi_two_module \
      GIT_COMMIT=c62a54a+cer-ratio-working-tree \
      bash "$REPO_ROOT/closed_loop/ctpi/run_ctpi_fasttrack_case_safe_20260903.sh"
  done
done
echo CSTAR_CER_RATIO_M12_HOUSE123_SEED12_COMPLETE
