                       ...
                       std::shared_ptr<gaden::PlaybackSimulation>& playback_out)
{
    ...
    std::sort(wind_files.begin(), wind_files.end(), [](const fs::path& a, const fs::path& b) {
        return std::stoull(a.filename().string().substr(15)) < std::stoull(b.filename().string().substr(15));
    });
    for (size_t i = 0; i < wind_files.size(); ++i) {
        if (wind_files[i].filename().string() != "wind_iteration_" + std::to_string(i)) {
            std::cerr << "RAW_QUERY_ERROR non_contiguous_numeric_wind_sequence\n";
            return false;
        }
    }
    if (wind_files.empty()) {
