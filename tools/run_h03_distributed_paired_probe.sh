#!/usr/bin/env bash
# Four offline H03 source x transport worlds for the frozen distributed route.
set -Ee -o pipefail
...
ROUTE_ROOT=$REPO_ROOT/evidence/m1r_h03_distributed_intervention_20260913/route
ROUTE=$ROUTE_ROOT/H03/history_route.csv
RULE=$ROUTE_ROOT/ROUTE_RULE.json
SELF=$REPO_ROOT/tools/run_h03_distributed_paired_probe.sh
...
expected={'schema':'M1R_DISTRIBUTED_HOST_REMOTE_RECEIPT_V1','resolved_commit':e['FREEZE_COMMIT'],
          'source_archive_sha256':digest}
for key,value in expected.items():
    if r.get(key)!=value: raise SystemExit('FREEZE_RECEIPT_MISMATCH:'+key)
rule=json.loads(pathlib.Path(e['RULE']).read_text()); route=pathlib.Path(e['ROUTE'])
...
if rule['station_count']!=3 or rule['all_station_measurement_operator_rank']!=2:
    raise SystemExit('DISTRIBUTED_GEOMETRY_CONTRACT_FAILED')
if rule['maximum_station_direction_condition_number']>=1.2 or rule['end_time_s']>240.0:
    raise SystemExit('DISTRIBUTED_ROUTE_LIMIT_FAILED')
PY
...
read -r SIM_TIME_S EXPECTED_FRAMES < <(python3 - <<'PY'
import csv,os
rows=list(csv.DictReader(open(os.environ['ROUTE']))); end=float(rows[-1]['t_sim_s']); sim=round(end+0.1,9)
print(sim,round(sim/0.1))
PY
)
export SIM_TIME_S EXPECTED_FRAMES
...
for p in pathlib.Path('/proc').iterdir():
    ...
    try:
        ...
        if any(pathlib.Path(x).name in {'filament_simulator','gsl_actionserver_node','vgr_sim_node'} for x in args[:3] if x): found.append((p.name,args[:3]))
...
run_case() {
  ...
  check_idle
  local work=/dev/shm/m1r_distributed_${FREEZE_COMMIT:0:12}_${id}
  ...
  [[ $(find "$raw_wind" -maxdepth 1 -type f -name 'wind_iteration_*' | wc -l) -eq 11 ]] || { echo WIND_FILE_COUNT_MISMATCH:$wind_config >&2; return 4; }
  for file in "$raw_wind"/wind_iteration_*; do number=${file##*_}; cp -- "$file" "$input/wind_iteration_${number}.csv_gaden"; done
  python3 - "$OUT_ROOT/$id/CASE_MANIFEST.json" "$id" "$wind_config" "$sx" "$sy" "$sz" "$raw_wind" <<'PY'
import datetime,hashlib,json,pathlib,sys,os
out,cid,wind,sx,sy,sz,wind_dir=sys.argv[1:]; files=sorted(pathlib.Path(wind_dir).glob('wind_iteration_*'))
m={'contract':'M1R_H03_DISTRIBUTED_PAIRED_WORLD_V1','created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
...
PY
  timeout 300s "$SIM_BIN" --ros-args \
    ...
    -p writeConcentrations:=false -p results_location:="$output" >"$OUT_ROOT/$id/sim.log" 2>&1
  [[ $(find "$output" -maxdepth 1 -type f -name 'iteration_*' | wc -l) -ge "$EXPECTED_FRAMES" ]] || { echo INCOMPLETE_GAS_FRAMES:$id >&2; return 5; }
  "$PY" "$EXTRACTOR" --env-root "$ENV_BASE/House03" --gas-results "$output" --route "$ROUTE" --helper "$HELPER" \
    --sensor-module "$SENSOR_ROOT/source_snapshot/sensor_model.py" --sensor-manifest "$SENSOR_ROOT/probes_v1/H03/sensor_manifest.json" \
    --raw-dt 0.1 --output "$OUT_ROOT/$id/measured_history.jsonl" \
  ...
  sha256sum "$OUT_ROOT/$id/CASE_MANIFEST.json" "$OUT_ROOT/$id/measured_history.jsonl" \
    "$OUT_ROOT/$id/candidate_forward_input.jsonl" "$OUT_ROOT/$id/sim.log" >"$OUT_ROOT/$id/FILE_SHA256"
}
...
m={'contract':'M1R_H03_DISTRIBUTED_PAIRED_RAW_V1','completed_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
   'freeze_commit':os.environ['FREEZE_COMMIT'],'files':files}
(root/'RAW_MANIFEST.json').write_text(json.dumps(m,indent=2)+'\n')
PY

echo DISTRIBUTED_H03_PHYSICAL_WORLDS_COMPLETE
