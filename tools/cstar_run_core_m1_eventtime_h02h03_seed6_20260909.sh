...
RUN_ROOT=${CSTAR_CORE_RUN_ROOT:-/mnt/hgfs/workspace/CSTAR_CORE_M1_EVENTTIME_H01_SEED6_20260908_R1}
BUILD_ROOT=/dev/shm/cstar_pmfs_core_m1_eventtime_build_20260908
...
GIT_COMMIT=e5e8984a1c8732adafd3a078e7e7d0a152d82dff

test -d "$RUN_ROOT/H01_seed6_A0"
test -d "$RUN_ROOT/H01_seed6_M1F"
test -x "$BUILD_ROOT/install/gsl_server/lib/gsl_server/gsl_actionserver_node"

for HOUSE in H02 H03; do
  for ARM in A0 M1F; do
    test ! -e "$RUN_ROOT/${HOUSE}_seed6_${ARM}"
    HOUSE="$HOUSE" SEED=6 SENSOR_SEED=12 ARM="$ARM" RUN_ROOT="$RUN_ROOT" REPO_ROOT="$REPO_ROOT" \
      ...
      DOMAIN_ID=223 TIMEOUT_SEC=240.0 METHOD=CTPI_G2_M1_M2 METHOD_FAMILY=ctpi_two_module \
      GIT_COMMIT="$GIT_COMMIT" \
      bash "$REPO_ROOT/closed_loop/ctpi/run_ctpi_fasttrack_case_safe_20260903.sh"
  done
done

printf '%s\n' "CSTAR_CORE_M1_EVENTTIME_H02H03_SEED6_COMPLETE"
