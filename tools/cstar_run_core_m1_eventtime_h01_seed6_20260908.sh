...
for ARM in A0 M1F; do
  HOUSE=H01 SEED=6 SENSOR_SEED=12 ARM="$ARM" RUN_ROOT="$RUN_ROOT" REPO_ROOT="$REPO_ROOT" \
    ...
    DOMAIN_ID=224 TIMEOUT_SEC=240.0 METHOD=CTPI_G2_M1_M2 METHOD_FAMILY=ctpi_two_module \
    GIT_COMMIT="$GIT_COMMIT" \
    bash "$REPO_ROOT/closed_loop/ctpi/run_ctpi_fasttrack_case_safe_20260903.sh"
done

printf '%s\n' "CSTAR_CORE_M1_EVENTTIME_H01_SEED6_COMPLETE"
