...
namespace gaden
{
    static thread_local PrecalculatedGaussian<1000> gaussian; // native default initialization preserved

    void ResetPrecalculatedGaussian()
    {
        gaussian = PrecalculatedGaussian<1000>();
    }

    // OpenMP workers own an independent thread-local cache.  The opt-in V3
    // seed is therefore applied lazily on each worker before its first move;
    // with no GADEN_RNG_SEED this branch is never entered and native behavior
    // remains byte-for-byte unchanged.
    static void InitializeSeededWorkerGaussian()
    {
        const char* seedText = std::getenv("GADEN_RNG_SEED");
...
#pragma omp parallel for
    ...
    {
        InitializeSeededWorkerGaussian();
