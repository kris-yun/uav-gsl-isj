...
PosteriorDiagnostics DualPosterior::update(
    ...
    // CAT: compute alpha from posterior concentration
    if (cat_delta > 0.0 && !log_map_.empty()) {
        computeCatAlpha();
    }
    ...
    for (std::size_t i = 0; i < sources_.size(); ++i) {
        const double q = clampProbability(
            OPGSLV3::ObservationModelEnsemble::hitProbability(model_, sources_[i], observation));
        // CAT + adaptive miss: tempered evidence
        // hit:  alpha * log(q)           — tempered by concentration
        // miss: alpha * p_d * log(1-q)   — tempered + adaptive
        double ell;
        const double alpha = (cat_delta > 0.0) ? cat_current_alpha : 1.0;
        if (use_adaptive_pd) {
            const double pd = std::clamp(pd_vals[i], 1e-6, 1.0 - 1e-6);
            ell = detected ? alpha * std::log(q) : alpha * pd * std::log1p(-q);
        } else {
            ell = detected ? alpha * std::log(q) : alpha * miss_weight * std::log1p(-q);
        }
        log_map_[i] += ell;
