...
namespace GSL::Utils::NQA
{
    ...
    std::vector<Node> Quadtree::fusedLeaves(int maxSize)
    {
...

        {
            ...
            // The original implementation used an unordered_set here.  Its
            // iteration order changes between processes, and the fusion
            // routine has many equal-distance candidates; that made the
            // PMFS support (and therefore OFF itself) nondeterministic for
            // an identical occupancy grid.  Order nodes by geometry so the
            // fusion schedule is reproducible.
            auto nodeOrder = [](const Node* a, const Node* b)
            {
            ...
            std::set<Node*, decltype(nodeOrder)> to_be_evaluated(nodeOrder);
            ...
            while (to_be_evaluated.size() != 0)
            {
                ...
                for (Node* neighbour : allNeighbours[current])
                {
                             ...
                             (current->size.x + neighbour->size.x) <= maxSize))
                    {
                        ...
                        float dist = std::abs((static_cast<float>(size.y) /
                                               static_cast<float>(size.x)) - 1.0f);
                        const bool tie = std::abs(dist - distanceFromSquare) <= 1e-6f;
                        const bool lexicographicallyEarlier =
                            best == nullptr ||
                            std::tie(neighbour->origin.x, neighbour->origin.y,
                                     neighbour->size.x, neighbour->size.y) <
                            std::tie(best->origin.x, best->origin.y,
                                     best->size.x, best->size.y);
                        if (dist < distanceFromSquare - 1e-6f || (tie && lexicographicallyEarlier))
                        {
