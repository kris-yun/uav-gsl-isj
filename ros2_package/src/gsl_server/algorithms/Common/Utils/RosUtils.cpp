...
namespace GSL::Utils
{
    ...
    static std::map<std::string, std::shared_ptr<rclcpp::Publisher<Marker>>> markerPublisherMap;
    static std::map<std::string, std::shared_ptr<rclcpp::Publisher<Marker>>> gridPublisherMap;
    static std::map<std::string, std::shared_ptr<rclcpp::Publisher<Marker>>> arrowPublisherMap;
    ...
    void publishDebugSingleMarker(Vector3 position, ColorRGBA color, const std::string& topic)
    {
        ...
        auto& publisherMap = markerPublisherMap;
    ...
    void publishDebugMarkers(Grid2D<std_msgs::msg::ColorRGBA> grid, const std::string& topic)
    {
        ...
        auto& publisherMap = gridPublisherMap;
    ...
    void publishDebugSingleArrow(Vector3 start, Vector3 end, std_msgs::msg::ColorRGBA color, const std::string& topic)
    {
        ...
        auto& publisherMap = arrowPublisherMap;
    ...
    void shutdownDebugRosResources()
    {
        // Destroy publishers before the ROS context is shut down.  The
        // original function-local static publishers survived until process
        // exit and then attempted Fast DDS teardown after rclcpp::shutdown().
        markerPublisherMap.clear();
        gridPublisherMap.clear();
        arrowPublisherMap.clear();
        debugNode.reset();
    }
} // namespace GSL::Utils
