...
namespace GSL
{
    ...
    void MovingState::goalDoneCallback(const rclcpp_action::ClientGoalHandle<NavigateToPose>::WrappedResult& result)
    {
        ...
        if (result.code == rclcpp_action::ResultCode::SUCCEEDED)
            ...
            outcome = "CANCELED";
        writeNavigationTrace("RESULT", outcome);
        if (result.code != rclcpp_action::ResultCode::SUCCEEDED)
        {
    ...
    void MovingState::sendGoal(const NavigateToPose::Goal& goal)
    {
        ...
        // Do not synchronously spin the owning algorithm node from inside its
        // state-machine callback.  That re-entrant wait starves the Python
        // VGR action server under the ordered-bundle workload and creates a
        // client-side timeout although the goal is later executed.  Dispatch
        // asynchronously; normal result callbacks and the active moving-state
        // timeout remain the single navigation completion contract.
        (void)nav_client->async_send_goal(goal, options);

        currentGoal = goal;
        active_navigation_goal_id_ = ++navigation_goal_counter_;
        writeNavigationTrace("SENT", "PENDING");
    ...
    void MovingState::Fail()
    {
        if (!currentGoal.has_value())
            return;
        writeNavigationTrace("RESULT", "FAIL_REPLAN");
        currentGoal = std::nullopt;
        nav_client->async_cancel_all_goals();
        algorithm->OnCompleteNavigation(GSLResult::Failure, previousState);
    }

    void MovingState::initNavigationTrace()
    {
        const std::string filepath = algorithm->getParam<std::string>("navigation_trace_file", "");
        if (filepath.empty())
            return;
        navigation_trace_file_.open(filepath, std::ios::out | std::ios::trunc);
        if (!navigation_trace_file_.is_open())
            return;
        navigation_trace_file_ << "run_uuid,goal_id,event,outcome,sim_time,goal_x,goal_y\n";
        navigation_trace_enabled_ = true;
    }

    void MovingState::writeNavigationTrace(const char* event, const char* outcome)
    {
        if (!navigation_trace_enabled_ || !navigation_trace_file_.is_open() || !currentGoal.has_value())
            return;
        const double sim_time = (algorithm->node->now() - algorithm->startTime).seconds();
        navigation_trace_file_ << algorithm->getParam<std::string>("run_uuid", "unknown") << ","
            << active_navigation_goal_id_ << "," << event << "," << outcome << "," << sim_time << ","
            << currentGoal->pose.pose.position.x << "," << currentGoal->pose.pose.position.y << "\n";
        navigation_trace_file_.flush();
    }
