...
namespace GSL
{
    ...
    void Algorithm::Initialize()
    {
        ...
        const auto experimentSeed = getParam<int64_t>("seed", 0);
        Utils::seedRandom(static_cast<uint64_t>(experimentSeed));
        ...
        // Extra wall-time safety net only.  The scientific budget is ROS
        // simulation time; deterministic planner pauses and 1x replay can make
        // wall time substantially longer without indicating a deadlock.
        const auto watchdog_seconds = static_cast<int>(
            std::ceil(resultLogging.maxSearchTime * 6.0 + 600.0));
        static auto exit_timer = node->create_wall_timer(std::chrono::seconds(watchdog_seconds),
                                                         []()
                                                         {
                                                             rclcpp::shutdown();
    ...
    void Algorithm::localizationCallback(const PoseWithCovarianceStamped::SharedPtr msg)
    {
        ...
        currentRobotPosition = Vector2(currentRobotPose.pose.pose.position.x, currentRobotPose.pose.pose.position.y);
    ...
    GSLResult Algorithm::checkSourceFound()
    {
        ...
        // 2. Distance from robot to source (disabled when distanceThreshold < 0)
        if (resultLogging.distanceThreshold > 0) {
