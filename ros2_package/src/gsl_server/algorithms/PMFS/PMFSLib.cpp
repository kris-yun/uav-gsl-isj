...
namespace GSL
{
                                ...
                                Vector2 startingPosition)
    {
...

        {
            ...
            for (int ii = 0; ii < grid.occupancy.size(); ii++) {
                if (grid.occupancy[ii] == Occupancy::Free) free_count++;
                else obs_count++;
            }
            GSL_INFO("[DEBUG] After reduceOccupancyMap: free={} obstacle={} total={}", free_count, obs_count, (int)grid.occupancy.size());
        }
        grid.metadata.numFreeCells = PruneUnreachableCells(grid.occupancy, grid.metadata, startingPosition);
        {
            ...
            GSL_INFO("[DEBUG] After PruneUnreachable: free={} obstacle={} total={}", free_count, obs_count, (int)grid.occupancy.size());
            GSL_INFO("[DEBUG] Grid dims: {}x{} scale={} cellSize={}", grid.metadata.dimensions.x, grid.metadata.dimensions.y, grid.metadata.scale, grid.metadata.cellSize);
            GSL_INFO("[DEBUG] Start pos: ({}, {})", startingPosition.x, startingPosition.y);
        }
                                   ...
                                   IF_GADEN(, PMFS_internal::GroundTruthWind& groundTruth))
    {
...
#ifdef USE_GADEN
        const int gmrf_req_size = gmrf.request ? static_cast<int>(gmrf.request->x.size()) : -1;
        const int gaden_req_size = groundTruth.request ? static_cast<int>(groundTruth.request->x.size()) : -1;
        GSL_INFO("[DEBUG-WIND] useGroundTruth={} gmrf_req_size={} gaden_req_size={}", useGroundTruth, gmrf_req_size, gaden_req_size);
        if (!useGroundTruth)
        {
#endif
            GSL_INFO("[DEBUG-WIND] Taking GMRF path");
        ...
        {
            // GADEN wind service - uses spin_until_future_complete to avoid
            // deadlock in single-threaded executor (was future.wait_for).
            // Called from functionQueue in main thread, so nested spin is safe.
            static int zero_wind_fallback_count = 0;
            bool wind_ok = false;
            if (groundTruth.client->service_is_ready()) {
                ...
                if (result == rclcpp::FutureReturnCode::SUCCESS) {
                    try {
                        ...
                        wind_ok = true;
                        GSL_INFO("[WIND] GADEN service OK ({} pts)", (int)groundTruth.request->x.size());
                    } catch (const std::exception& e) {
                        GSL_ERROR("[WIND] GADEN response parse error: {}", e.what());
                    }
                } else {
                    GSL_WARN("[WIND] spin_until_future_complete returned: {}",
                             result == rclcpp::FutureReturnCode::TIMEOUT ? "TIMEOUT" : "INTERRUPTED");
                }
            } else {
                GSL_WARN("[WIND] GADEN service not ready");
            }
            if (!wind_ok) {
                zero_wind_fallback_count++;
                GSL_ERROR("[WIND] GADEN service FAILED (fallback_count={}). "
                          "Wind field NOT loaded - hit probability kernel uses default orientation.",
                          zero_wind_fallback_count);
            } else {
                if (zero_wind_fallback_count > 0) {
                    GSL_INFO("[WIND] GADEN service recovered after {} fallbacks", zero_wind_fallback_count);
                }
                zero_wind_fallback_count = 0;
            }
        }
#endif
    }
