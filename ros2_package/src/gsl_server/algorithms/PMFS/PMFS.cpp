...
namespace GSL
{
    ...
    void PMFS::declareParameters()
    {
        ...
        if (pfdiMode != "off" && pfdiMode != "cer_m1" && pfdiMode != "cer_m1_m2" &&
            ...
            pfdiMode != "cer_core_eventtime_m1" && pfdiMode != "cer_core_invariant_m1" && pfdiMode != "cer_core_robust_m1" &&
            ...
            throw std::invalid_argument("pfdi_mode must be off, cer_m1, cer_m1_m2, cer_ratio_m1, cer_ratio_m1_m2, cer_core_m1, cer_core_seq_m1, cer_core_stop_m1, cer_core_ensemble_m1, cer_core_eventtime_m1, cer_core_invariant_m1, cer_core_robust_m1, cer_core_phic_m1, cpir_m1, cpir_a1, cpir_a2, cpir_a3, cpir_m1_m3, ctpi_f00, ctpi_f01, ctpi_f10, ctpi_f11, sd, tadm, joint, al, pc_aci, me_aci, me_aci_shadow, ec_edcl, or ec_edcl_shadow");
        ...
        eventEvidenceTransportRobustPool = pfdiMode == "cer_core_robust_m1";
        const bool sensorFopdtMode = pfdiMode == "cer_core_phic_m1";
        if (sensorFopdtMode)
        {
            // PHIC uses raw exposure context and a fixed FOPDT state.  Keep
            // this branch isolated so frozen CORE M1G/M1H results do not
            // silently change.
            eventEvidenceCenteredLogOdds = false;
            eventEvidenceSequentialAssimilation = true;
            eventEvidenceAfterWarmupOnly = true;
            eventEvidenceTransportLogPool = false;
            eventEvidenceTransportRobustPool = true;
        }
        eventEvidenceTransportReplicas = (pfdiMode == "cer_m1_m2" || pfdiMode == "cer_ratio_m1_m2" ||
                                          pfdiMode == "cer_core_ensemble_m1" ||
                                          pfdiMode == "cer_core_eventtime_m1" ||
                                          pfdiMode == "cer_core_invariant_m1" || pfdiMode == "cer_core_robust_m1" ||
                                          pfdiMode == "cer_core_phic_m1") ? 3 : 1;
...
namespace GSL
{
    ...
    void PMFS::processGasAndWindMeasurements(double concentration, double windSpeed, double windDirection)
    {
        ...
        // M1P uses one terminal block per physical sensing stop because the
        // chosen-position intervention is unchanged across the eight internal
        // measurement blocks. Treating those dependent blocks as independent
