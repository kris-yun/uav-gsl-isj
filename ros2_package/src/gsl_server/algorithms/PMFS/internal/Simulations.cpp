...
namespace GSL::PMFS_internal
{
    ...
    void Simulations::configureM1RSourceQuadrature(bool enabled)
    {
        if (enabled && (!nativeDeterministicRng || !eventEvidenceEnabled || !eventEvidenceContrastiveRatio ||
                        eventEvidenceCenteredLogOdds || eventEvidenceSequentialAssimilation ||
                        eventEvidenceTransportLogPool || eventEvidenceTransportRobustPool ||
                        eventEvidenceSensorFopdt))
            throw std::invalid_argument("M1R_SOURCE_QUADRATURE_REQUIRES_NATIVE_DETERMINISTIC_CER_RATIO");
        m1rSourceQuadratureEnabled = enabled;
        if (enabled)
            GSL_INFO("M1R source quadrature: U=4, equal weights, fixed point per complete PF, "
                     "finite rectangular approximation, source_dim=2, unchanged transport K={}, "
                     "legacy planner forward retained, forward cost multiplier=5", eventEvidenceTransportReplicas);
    }
...
#pragma omp parallel for schedule(dynamic)
    ...
    Simulations::SimulationResult Simulations::runSimulation(std::vector<LeafScore>& scores, size_t index)
    {
        ...
        {
            if (!nativeDeterministicRng || !eventEvidenceEnabled || !eventEvidenceContrastiveRatio ||
                eventEvidenceCenteredLogOdds || eventEvidenceSequentialAssimilation ||
                eventEvidenceTransportLogPool || eventEvidenceTransportRobustPool || eventEvidenceSensorFopdt)
                throw std::runtime_error("M1R_SOURCE_QUADRATURE_CONFIGURATION_CHANGED");

            // Keep result.hitMap exactly as generated by the legacy code
            // above. Only the source scoring maps/context use quadrature.
            // Each point persists through the complete warmup + recording
            // simulation; U never enters the transport random key.
            const Vector2 start = measuredHitProb.metadata.indicesToCoordinates(
                node->origin.x, node->origin.y, false);
            const Vector2 end = measuredHitProb.metadata.indicesToCoordinates(
                node->origin.x + node->size.x, node->origin.y + node->size.y, false);
            constexpr std::array<std::array<float, 2>, 4> supportFractions{{
                {{0.25f, 0.25f}}, {{0.25f, 0.75f}}, {{0.75f, 0.25f}}, {{0.75f, 0.75f}}}};
            result.transportMemberHitMaps.assign(replicas, std::vector<float>(result.hitMap.size(), 0.0f));
            result.transportMemberExposureMaps.assign(replicas, std::vector<float>(result.hitMap.size(), 0.0f));
            result.sourcePointComponents.reserve(supportFractions.size());
            for (const auto& fraction : supportFractions)
            {
                SourcePointComponent component;
                component.point = Vector2(start.x + fraction[0] * (end.x - start.x),
                                          start.y + fraction[1] * (end.y - start.y));
                const Vector2Int sourceIndices = measuredHitProb.metadata.coordinatesToIndices(component.point);
                if (!std::isfinite(component.point.x) || !std::isfinite(component.point.y) ||
                    component.point.x < start.x || component.point.x >= end.x ||
                    component.point.y < start.y || component.point.y >= end.y ||
                    !measuredHitProb.metadata.indicesInBounds(sourceIndices) ||
                    !measuredHitProb.freeAt(sourceIndices))
                    throw std::runtime_error("M1R_SOURCE_QUADRATURE_POINT_OUTSIDE_FREE_REGION");
                component.transportMemberHitMaps.assign(replicas, std::vector<float>(result.hitMap.size(), 0.0f));
                component.transportMemberExposureMaps.assign(replicas, std::vector<float>(result.hitMap.size(), 0.0f));
                for (int replica = 0; replica < replicas; ++replica)
                {
                    EventKeyedTransportRng pointTransportRng(EventKey{
                        nativeRandomSeed, nativeSourceUpdateId, static_cast<uint64_t>(replica), nativeTransportSubstream});
                    SimulationSource fixedSource(component.point, measuredHitProb.metadata);
                    auto& pointMap = component.transportMemberHitMaps[replica];
                    auto& pointExposure = component.transportMemberExposureMaps[replica];
                    simulateSourceInPosition(fixedSource, pointMap, true, settings.iterationsToRecord,
                                             settings.deltaTime, settings.noiseSTDev, &pointExposure, &pointTransportRng);
                    if (pointMap.size() != result.hitMap.size() || pointExposure.size() != result.hitMap.size())
                        throw std::runtime_error("M1R_SOURCE_QUADRATURE_FORWARD_MAP_SIZE");
                    for (size_t cell = 0; cell < pointMap.size(); ++cell)
                    {
                        result.transportMemberHitMaps[replica][cell] += 0.25f * pointMap[cell];
                        result.transportMemberExposureMaps[replica][cell] += 0.25f * pointExposure[cell];
                    }
                }
                result.sourcePointComponents.push_back(std::move(component));
            }
        }
...
#pragma omp parallel for schedule(dynamic)
    ...
    void Simulations::exportContrastiveEventAttribution(
        ...
        int sourcePointIndex, long double sourcePointWeight)
    {
        ...
        std::filesystem::create_directories(contextBankExportDirectory);
        const std::string path = contextBankExportDirectory +
            (sourcePointIndex >= 0 ? "/contrastive_source_point_event_attribution.csv" : "/contrastive_event_attribution.csv");
        ...
        if (!existed || std::filesystem::file_size(path) == 0)
        {
            ...
            if (sourcePointIndex >= 0)
                output << ",source_point_index,source_point_weight,event_window_start,source_quadrature_context";
            output << '\n';
        }
        if (sourcePointIndex >= 0 && (sourcePointIndex >= 4 || sourcePointWeight != 0.25L ||
                                     memberMaps.size() != eventEvidenceContext.size() ||
                                     exposureMaps.size() != memberMaps.size()))
            throw std::runtime_error("M1R_SOURCE_POINT_ATTRIBUTION_STRUCTURE");
        for (size_t memberIndex = 0; memberIndex < memberMaps.size(); ++memberIndex)
        {
            ...
            for (size_t eventIndex = 0; eventIndex < eventEvidence.size(); ++eventIndex)
            {
                ...
                if (sourcePointIndex >= 0 && (event.cell >= memberMap.size() ||
                                              eventEvidenceContext[memberIndex].size() != eventEvidence.size()))
                    throw std::runtime_error("M1R_SOURCE_POINT_ATTRIBUTION_EVENT_STRUCTURE");
                ...
                output << contextBankExportRunUUID << ',' << contextBankSourceUpdateId << ',' << stableID << ','
                       << std::setprecision(sourcePointIndex >= 0 ? std::numeric_limits<long double>::max_digits10 : 9)
                       << source.x << ',' << source.y << ',' << memberIndex << ','
                ...
                if (sourcePointIndex >= 0)
                    output << ',' << sourcePointIndex << ',' << sourcePointWeight << ',' << eventEvidenceWindowStart
                           << ",source_U_mean_then_candidate_mean_within_transport_K";
                output << '\n';
            }
        }
        if (sourcePointIndex >= 0)
        {
            output.flush();
            if (!output)
                throw std::runtime_error("M1R_SOURCE_POINT_ATTRIBUTION_WRITE_FAILED");
        }
    }

    void Simulations::exportM1RSourcePointScores(const SimulationResult& result)
    {
        if (!m1rSourceQuadratureEnabled || !contextBankExportEnabled || contextBankExportDirectory.empty())
            return;
        if (!result.valid || result.leaf == nullptr || result.sourcePointComponents.size() != 4)
            throw std::runtime_error("M1R_SOURCE_POINT_SCORE_EXPORT_STRUCTURE");
        std::lock_guard<std::mutex> lock(contextBankExportMutex);
        std::filesystem::create_directories(contextBankExportDirectory);
        const std::string path = contextBankExportDirectory + "/source_point_component_scores.csv";
        const bool existed = std::filesystem::exists(path);
        std::ofstream output(path, std::ios::out | std::ios::app);
        if (!output)
            throw std::runtime_error("M1R_SOURCE_POINT_SCORE_EXPORT_OPEN_FAILED");
        if (!existed || std::filesystem::file_size(path) == 0)
            output << "run_uuid,source_update_id,candidate_id,origin_i,origin_j,size_i,size_j,source_point_index,source_x,source_y,source_weight,transport_members,component_likelihood,weighted_component_likelihood,region_mixture_likelihood,legacy_forward_calls,fixed_point_forward_calls,source_dim,quadrature_semantics\n";
        output << std::setprecision(std::numeric_limits<long double>::max_digits10);
        for (size_t index = 0; index < result.sourcePointComponents.size(); ++index)
        {
            const SourcePointComponent& component = result.sourcePointComponents[index];
            output << contextBankExportRunUUID << ',' << contextBankSourceUpdateId << ',' << p2StableID(result.leaf) << ','
                   << result.leaf->origin.x << ',' << result.leaf->origin.y << ','
                   << result.leaf->size.x << ',' << result.leaf->size.y << ',' << index << ','
                   << component.point.x << ',' << component.point.y << ',' << component.weight << ','
                   << component.transportMemberHitMaps.size() << ',' << component.sourceProb << ','
                   << component.weight * component.sourceProb << ',' << result.sourceProb << ','
                   << eventEvidenceTransportReplicas << ',' << 4 * eventEvidenceTransportReplicas
                   << ",2,four_fixed_rectangle_points_finite_approximation_not_exact_region_integral\n";
        }
        output.flush();
        if (!output)
            throw std::runtime_error("M1R_SOURCE_POINT_SCORE_EXPORT_WRITE_FAILED");
    }
    ...
    void Simulations::initializeContrastiveEventContext(const std::vector<SimulationResult>& results)
    {
        ...
        for (const SimulationResult* result : ordered)
        {
            ...
            if (m1rSourceQuadratureEnabled &&
                (result->sourcePointComponents.size() != 4 || result->transportMemberExposureMaps.size() != memberCount))
                throw std::runtime_error("M1R_SOURCE_QUADRATURE_CONTEXT_STRUCTURE");
        ...
        GSL_INFO("CER contrastive context initialized: events={}, candidates={}, transport_members={}, fixed_ratio_weight=1, context_within_member=true, centered_log_odds={}, transport_pool={}",
                 ...
                 eventEvidenceTransportRobustPool ? "robust_log_variance" :
        ...
        const std::vector<std::vector<float>>& transportMemberExposureMaps) const
    {
        ...
        long double meanSquaredLogLikelihood = 0.0L;
        for (size_t memberIndex = 0; memberIndex < transportMemberHitMaps.size(); ++memberIndex)
        {
            ...
            else if (eventEvidenceTransportRobustPool)
            {
                const long double members = static_cast<long double>(transportMemberHitMaps.size());
                meanLogLikelihood += logLikelihood / members;
                meanSquaredLogLikelihood += (logLikelihood * logLikelihood) / members;
            }
            else
        ...
        else if (eventEvidenceTransportRobustPool)
        {
            const long double variance = std::max(0.0L,
                meanSquaredLogLikelihood - meanLogLikelihood * meanLogLikelihood);
            // Entropic-risk correction with fixed unit risk aversion: retain
            // transport-consensus evidence while discounting member-disagreement.
            mixtureLikelihood = std::exp(meanLogLikelihood - 0.5L * variance);
        }
                                                     ...
                                                     std::vector<LeafScore>& scores)
    {
        ...
        if (contextBankExportEnabled && !contextBankAttributionExportedForUpdate)
        {
            for (const SimulationResult& result : results)
            {
                if (!result.valid || result.leaf == nullptr)
                    continue;
                // The U-mean map constructs context only. It is not a
                // scoring atom; point-level atoms are exported after scoring.
                if (m1rSourceQuadratureEnabled)
                    continue;
                const Vector2 candidatePoint = measuredHitProb.metadata.indicesToCoordinates(
        ...
        for (SimulationResult& result : results)
        {
            if (!result.valid || result.leaf == nullptr)
                continue;
            long double eventLikelihood;
            if (m1rSourceQuadratureEnabled)
            {
                if (result.sourcePointComponents.size() != 4 ||
                    result.transportMemberHitMaps.size() != static_cast<size_t>(eventEvidenceTransportReplicas) ||
                    result.transportMemberExposureMaps.size() != result.transportMemberHitMaps.size() ||
                    eventEvidenceContext.size() != result.transportMemberHitMaps.size())
                    throw std::runtime_error("M1R_SOURCE_QUADRATURE_SCORE_STRUCTURE");
                eventLikelihood = 0.0L;
                long double weightSum = 0.0L;
                for (SourcePointComponent& component : result.sourcePointComponents)
                {
                    if (component.weight != 0.25L ||
                        component.transportMemberHitMaps.size() != result.transportMemberHitMaps.size() ||
                        component.transportMemberExposureMaps.size() != result.transportMemberHitMaps.size())
                        throw std::runtime_error("M1R_SOURCE_QUADRATURE_COMPONENT_STRUCTURE");
                    for (size_t member = 0; member < component.transportMemberHitMaps.size(); ++member)
                        if (component.transportMemberHitMaps[member].size() != measuredHitProb.data.size() ||
                            component.transportMemberExposureMaps[member].size() != measuredHitProb.data.size())
                            throw std::runtime_error("M1R_SOURCE_QUADRATURE_COMPONENT_MAP_SIZE");
                    // Preserve the existing scorer and shared K-specific
                    // context. Marginalize U only after the entire event
                    // prefix and transport mixture have been scored.
                    component.sourceProb = sourceProbFromContrastiveEvents(
                        component.transportMemberHitMaps, component.transportMemberExposureMaps);
                    if (!(std::isfinite(component.sourceProb) && component.sourceProb > 0.0L))
                        throw std::runtime_error("M1R_SOURCE_QUADRATURE_COMPONENT_LIKELIHOOD_INVALID");
                    eventLikelihood += component.weight * component.sourceProb;
                    weightSum += component.weight;
                }
                if (weightSum != 1.0L || !(std::isfinite(eventLikelihood) && eventLikelihood > 0.0L))
                    throw std::runtime_error("M1R_SOURCE_QUADRATURE_MIXTURE_INVALID");
            }
            else
                eventLikelihood = sourceProbFromContrastiveEvents(
                    result.transportMemberHitMaps, result.transportMemberExposureMaps);
            result.sourceProb = eventLikelihood;
            if (m1rSourceQuadratureEnabled)
            {
                const std::string stableID = p2StableID(result.leaf);
                for (size_t pointIndex = 0; pointIndex < result.sourcePointComponents.size(); ++pointIndex)
                {
                    const SourcePointComponent& component = result.sourcePointComponents[pointIndex];
                    exportContrastiveEventAttribution(stableID, component.point,
                        component.transportMemberHitMaps, component.transportMemberExposureMaps,
                        static_cast<int>(pointIndex), component.weight);
                }
                exportM1RSourcePointScores(result);
            }
            if (eventEvidenceSequentialAssimilation && eventEvidenceSequentialPosteriorValid)
            {
