...
// Header-only, ROS-independent, truth-independent. The adapter supplies:
//   observed ppm prefix in the domain chosen by the engine;
//   candidate predictive prefixes [source][member][time] in the same domain;
//   geometry-only prior mass;
//   the fixed physical gas threshold used as log-level reference.
//
// Scientific modules are isolated so an ablation removes one mechanism only.
...
namespace GSL::PMFS_internal::pfdei
{
    ...
    struct SensorInverseConfig
    {
        ...
        double tau = 1.2;
        int delaySamples = 2;
        // Runtime raw-double default. Historical six-decimal replay must pass
        ...
        const SensorInverseConfig& cfg = {})
    {
        ...
        const std::size_t skip = cfg.delaySamples > 0
            ? static_cast<std::size_t>(cfg.delaySamples - 1) : 0U;
        if (raw.size() <= skip)
    ...
    inline std::vector<double> pathVector(
        ...
        bool temporal)
    {
        ...
        const double sl = std::sqrt(static_cast<double>(x.size()));
        ...
        for (std::size_t i = 0; i < x.size(); ++i)
        {
            ...
            // IMPORTANT: level normalization is invariant across FULL and -M4.
            // M4 removal deletes only the adjacent-difference channel.
            level[i] = std::log1p(x[i] / referencePpm) / sl;
        }
        if (!temporal) return level;

        const double sd = std::sqrt(static_cast<double>(x.size() - 1));
        std::vector<double> out;
        out.reserve(level.size() + level.size() - 1);
        out.insert(out.end(), level.begin(), level.end());
        for (std::size_t i = 1; i < x.size(); ++i)
        {
            const double a = std::log1p(x[i] / referencePpm);
            const double b = std::log1p(x[i - 1] / referencePpm);
            out.push_back((a - b) / sd);
        }
        return out;
    }
    ...
    inline double pathDistance(
        const std::vector<double>& a,
        const std::vector<double>& b,
        double referencePpm,
        bool temporal)
    {
        return euclidean(pathVector(a, referencePpm, temporal),
                         pathVector(b, referencePpm, temporal));
    }
    ...
    inline void validateTensor(const std::vector<double>& observed, const Tensor& predicted)
    {
        ...
        {
            if (source.size() != M)
                throw std::invalid_argument("member count drift");
            for (const auto& member : source)
            {
                if (member.size() != observed.size())
                    throw std::invalid_argument("time support mismatch");
                for (double v : member)
                    if (!(v >= 0.0) || !std::isfinite(v))
    ...
    inline Tensor coherenceScramble(const Tensor& input)
    {
        ...
        Tensor out = input;
        const std::size_t S = input.size();
        const std::size_t M = input[0].size();
        const std::size_t T = input[0][0].size();
    ...
    inline std::vector<double> energyScores(
        ...
        bool temporal)
    {
        ...
        const std::size_t S = predicted.size();
        const std::size_t M = predicted[0].size();
    ...
    inline std::vector<double> meanFieldScores(
        const std::vector<double>& observed,
        const Tensor& predicted,
        double referencePpm,
        bool temporal)
    {
        ...
        const std::size_t S = predicted.size();
        const std::size_t M = predicted[0].size();
        const std::size_t T = observed.size();
        std::vector<double> out(S, 0.0);
        for (std::size_t s = 0; s < S; ++s)
        {
    ...
    inline std::vector<double> normalMidrankEvidenceLowerIsBetter(
        const std::vector<double>& values)
    {
        const std::size_t n = values.size();
        if (n < 2) throw std::invalid_argument("need >=2 scores");
        for (double v : values)
            if (!std::isfinite(v)) throw std::invalid_argument("nonfinite score");
        ...
        while (begin < n)
        {
            ...
            while (end < n && std::abs(values[order[end]] - values[order[begin]]) <= tol)
                ++end;
    ...
    inline std::vector<long double> reversiblePosterior(
        std::vector<long double> prior,
        const std::vector<double>& z)
    {
        prior = normalizeMass(std::move(prior));
        if (prior.size() != z.size())
            throw std::invalid_argument("posterior shape mismatch");
    ...
    inline Result compute(
        const std::vector<double>& observedPpm,
        const Tensor& predictedPpm,
        ...
        Mode mode)
    {
        ...
        try
        {
            ...
            out.scores = (mode == Mode::AblateNuisance)
                ? meanFieldScores(observedPpm, p, referencePpm, temporal)
                : energyScores(observedPpm, p, referencePpm, temporal);
            ...
            out.selectedSource = static_cast<int>(std::distance(
                out.candidateMass.begin(),
                std::max_element(out.candidateMass.begin(), out.candidateMass.end())));
            out.valid = true;
            out.reason = "OK";
        }
