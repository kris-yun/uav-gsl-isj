...
Date: 2026-08-31
Status: THEORY FROZEN / RUNTIME SOURCE ALIGNED / VM BUILD PASS / BANK NOT OPENED
...
`K` is one exchangeable joint nuisance atom (within-carrier source placement,
height, and transport seed). It is NOT a temporal state. Coherent-K and
dynamic Markov-K are permanently rejected.
...
The reviewed worktree now implements this score in explicit `cpir_a3` mode.
`cpir_a1`, `cpir_a2`, and the historical `cpir_m1` alias retain the count-only
score as the required comparator. The isolated VM Release build passed; actual
bank-backed runtime parity is still pending.
...
The reviewed worktree now applies this interface with the frozen uniform
geometry-only pre-gas cell reference. It also fails closed unless the cell
manifest is exactly the runtime free-cell set. The VM build passed; runtime
...
6. Closed loop is authorized only after the nested evidence is understood and
   the source-aligned runtime passes VM build, synthetic smoke, and formula
   parity. No bank regeneration is implied by M2/M3 or the projection changes.
